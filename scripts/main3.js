...
function switchImage(){
    if(+realSpeed.textContent===0){                         //将字符串转为数字进行判零
...
function setSpeed() {
    let speed = prompt("请输入速度阈值（0~255）。");
        ...
        this.device.gatt
            ...
            .then((characteristic) => {
                characteristic.addEventListener("characteristicvaluechanged", (e) => {
                    ...
                    let tSpeed=data.getFloat32(0,true).toFixed(0);//读取速度阈值数据
                    let rSpeed=data.getFloat32(4,true).toFixed(2);//读取当前速度数据
